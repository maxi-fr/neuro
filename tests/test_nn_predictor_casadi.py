...
from neuro.nn_predictor_casadi import NNSymbolicModel, _mlp_forward_ca  # noqa: E402
from neuro.prediction import AutoregressivePredictor, get_activation  # noqa: E402
from neuro.predictor.artifact import Activation, MLPArtifact  # noqa: E402
...
def _layers_of(mlp: eqx.nn.MLP) -> tuple[tuple[FloatArray, FloatArray], ...]:
    """Pull numpy ``(weight, bias)`` pairs out of an Equinox MLP, in forward-pass order."""
    return tuple((np.asarray(lay.weight, np.float64), np.asarray(lay.bias, np.float64)) for lay in mlp.layers)
...
def _jax_horizon_rollout(
    art: MLPArtifact, mlp: eqx.nn.MLP, y_ctx: FloatArray, u_ctx: FloatArray, u_future: FloatArray
) -> FloatArray:
    ...
    y_model = np.asarray(predictor(jnp.asarray(x_in))).reshape(horizon, art.n_channels)
...
def _build_artifact(
    ...
    The Equinox MLP is the reference the CasADi port is pinned against; the artifact carries
    the very same weights as plain numpy arrays.
    """
...
def _build_projection_artifact(
    ...
    n_controls: int,
    activation: Activation = "relu",
) -> tuple[Path, eqx.nn.MLP]:
    """Save a tiny artifact whose model runs in a ``k``-dim PCA latent space; return it and its MLP.
...
def test_mlp_forward_matches_jax(
    ...
    fn = ca.Function("f", [x_sym], [_mlp_forward_ca(x_sym, mlp_artifact.layers, activation)])
...
def test_multistep_rollout_matches_jax_model(
    ...
    artifact, mlp, _scalers = _build_artifact(
        tmp_path, n_y, n_u, horizon, hidden_size, depth, n_channels, n_controls, activation
    )
    model = NNSymbolicModel.from_artifact(artifact)
    art = MLPArtifact.load(artifact)
    ...
    y_pred_jax = _jax_horizon_rollout(art, mlp, y_ctx, u_ctx, u_future)  # (n_eeg, horizon)
...
def test_f_out_decodes_latent_state_to_eeg(tmp_path: Path) -> None:
    ...
    artifact, _mlp = _build_projection_artifact(tmp_path, n_y, n_u, 3, 4, 2, k, n_eeg, n_controls)
    model = NNSymbolicModel.from_artifact(artifact)
    art = MLPArtifact.load(artifact)
...
def test_multistep_rollout_matches_jax_model_with_projection(tmp_path: Path, activation: Activation) -> None:
    ...
    artifact, mlp = _build_projection_artifact(
        tmp_path, n_y, n_u, horizon, hidden, depth, k, n_eeg, n_controls, activation
    )
    model = NNSymbolicModel.from_artifact(artifact)
    art = MLPArtifact.load(artifact)
    ...
    y_pred_jax = _jax_horizon_rollout(art, mlp, y_ctx, u_ctx, u_future)  # (n_eeg, horizon), decoded
...
# --------------------------------------------------------------------------------------
# Part-1 temporary shim test: the NumPy artifact and the Equinox predictor coexist only in
# this commit, so this is the one chance to check the NumPy AR loop directly. Delete it in
# Part 5 together with `neuro.prediction`.
# --------------------------------------------------------------------------------------
@pytest.mark.parametrize("activation", ["relu", "tanh", "softplus"])
@pytest.mark.parametrize("projection", [False, True])
def test_numpy_rollout_matches_equinox_rollout(
    tmp_path: Path,
    activation: Activation,
    projection: bool,  # noqa: FBT001
) -> None:
    """``MLPArtifact.rollout`` (NumPy) reproduces the Equinox ``AutoregressivePredictor`` rollout."""
    n_y, n_u, horizon, n_controls = 3, 2, 6, 2
    if projection:
        artifact, mlp = _build_projection_artifact(tmp_path, n_y, n_u, horizon, 4, 2, 2, 5, n_controls, activation)
    else:
        artifact, mlp, _ = _build_artifact(tmp_path, n_y, n_u, horizon, 5, 2, 3, n_controls, activation)
    art = MLPArtifact.load(artifact)

    rng = np.random.default_rng(_SEED + 11)
    hist = max(n_y, n_u)
    y_hist = rng.standard_normal((hist, art.n_eeg_channels))
    u_hist = rng.standard_normal((hist, n_controls))
    u_future = rng.standard_normal((horizon, n_controls))

    got = art.rollout(art.prime(y_hist, u_hist), u_future)
    want = _jax_horizon_rollout(art, mlp, y_hist.T, u_hist, u_future).T
