...
from neuro.predictor.losses import curriculum_mask


def _length(mask: torch.Tensor) -> int:
    """length."""
    return int(mask.sum())
...
def test_mask_is_prefix_of_ones() -> None:
    ...
    for epoch in range(0, 120, 7):
        mask = curriculum_mask(epoch, horizon, start_epoch=10, end_epoch=90)
        length = _length(mask)
        assert mask.shape == (horizon,)
        assert np.array_equal(mask.numpy(), np.array([1.0] * length + [0.0] * (horizon - length)))
...
def test_holds_at_one_before_start_then_grows_to_horizon() -> None:
    ...
    def length_at(epoch: int) -> int:
        """length at."""
        return _length(curriculum_mask(epoch, horizon, start_epoch=start_epoch, end_epoch=end_epoch))
...
def test_zero_width_window_jumps_to_horizon() -> None:
    ...
    horizon, pivot = 20, 30
    assert _length(curriculum_mask(pivot, horizon, start_epoch=pivot, end_epoch=pivot)) == 1
    assert _length(curriculum_mask(pivot + 1, horizon, start_epoch=pivot, end_epoch=pivot)) == horizon


def test_curriculum_max_steps_caps_the_ramp() -> None:
    """The curriculum ramps ``1 -> max_steps``, never past it.
    ...
    lengths = [_length(curriculum_mask(epoch, horizon, cap, 0, 80)) for epoch in (0, 20, 40, 80, 1000)]
    assert lengths[0] == 1
    assert lengths == sorted(lengths)
    assert max(lengths) == cap
    for epoch in (0, 40, 1000):
        mask = curriculum_mask(epoch, horizon, cap, 0, 80)
        assert mask.shape == (horizon,)
        assert mask[cap:].sum() == 0


def test_curriculum_max_steps_of_one_is_one_step_training() -> None:
    """A ramp from 1 to 1 never grows, so ``max_steps=1`` is plain one-step training."""
    assert all(_length(curriculum_mask(epoch, 20, 1, 0, 80)) == 1 for epoch in (0, 40, 80, 1000))


def test_curriculum_max_steps_none_ramps_to_the_full_horizon() -> None:
    """Leaving the cap unset ramps over the whole horizon."""
    horizon, start, end = 20, 10, 90
    for epoch in (0, 5, 10, 50, 90, 200):
        capped = curriculum_mask(epoch, horizon, horizon, start, end)
        torch.testing.assert_close(curriculum_mask(epoch, horizon, None, start, end), capped)
...
def test_curriculum_max_steps_above_horizon_is_clamped() -> None:
    ...
    horizon = 20
    assert _length(curriculum_mask(1000, horizon, 999, 0, 80)) == horizon
