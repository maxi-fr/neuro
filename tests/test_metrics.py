...
def test_evaluate_rollouts_pools_its_own_per_step_curve() -> None:
    ...
    model = AutoregressiveMLP.from_artifact(
        _tiny_mlp_artifact(n_y=4, n_u=2, horizon=horizon, n_eeg=n_eeg, n_controls=n_controls)
    )
...
def test_log_energy_is_zero_when_the_prediction_matches() -> None:
    ...
    horizon, n_eeg, n_controls = 6, 3, 2
    model = AutoregressiveMLP.from_artifact(
        _silent_mlp_artifact(n_y=4, n_u=2, horizon=horizon, n_eeg=n_eeg, n_controls=n_controls)
    )
...
def test_log_energy_rejects_a_horizon_shorter_than_one_window() -> None:
    model = AutoregressiveMLP.from_artifact(_tiny_mlp_artifact(n_y=4, n_u=2, horizon=6, n_eeg=3, n_controls=2))
