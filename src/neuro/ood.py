...
def extract_state_action_pairs(
    ...
    *,
    ...
    n_y : int, default=1
        Past Observable Frame steps in the measurement history window.
    n_u : int, default=0
        Past Control Current steps in the control history window. When 0, pairs the measurement
        window with the candidate instantaneous Control Current.
    ...
    for u, y in trajectories:
        ...
        if n_y <= 1:
            ...
            y_windows = np.lib.stride_tricks.sliding_window_view(y_arr[:end_idx], n_y, axis=0)
            y_win = y_windows[start_idx - (n_y - 1) :].reshape(len(indices), -1)

        if n_u <= 0 or n_u == 1:
            u_win = u_arr[indices]
        else:
            u_windows = np.lib.stride_tricks.sliding_window_view(u_arr[:end_idx], n_u, axis=0)
            u_win = u_windows[start_idx - (n_u - 1) :].reshape(len(indices), -1)

        z_mat = np.hstack([y_win, u_win])
        z_list.append(z_mat)
...
def _build_mpc_knot_query(  # noqa: PLR0913 -- knot extraction requires log arrays, decision index, knot index, and window sizes
    all_pred_y: FloatArray,
    all_plan_u: FloatArray,
    all_applied_u: FloatArray,
    d: int | np.integer[Any],
    k: int | np.integer[Any],
    *,
    n_y: int,
    n_u: int,
) -> FloatArray:
    """Construct one state-action query vector at decision d and lookahead knot k."""
    if n_y <= 1:
        y_win = all_pred_y[d, k]
    else:
        y_win_items = [
            all_pred_y[d + step_offset, 0] if step_offset <= 0 else all_pred_y[d, step_offset]
            for step_offset in range(k - (n_y - 1), k + 1)
        ]
        y_win = np.concatenate(y_win_items, axis=0)

    if n_u <= 0 or n_u == 1:
        u_win = all_plan_u[d, k]
    else:
        u_win_items = [
            all_applied_u[d + step_offset] if step_offset < 0 else all_plan_u[d, step_offset]
            for step_offset in range(k - (n_u - 1), k)
        ]
        u_win_items.append(all_plan_u[d, k])
        u_win = np.concatenate(u_win_items, axis=0)

    return np.concatenate([y_win.reshape(-1), u_win.reshape(-1)])
...
def extract_mpc_rollout_queries(
    ...
    *,
    n_y: int = 1,
    n_u: int = 0,
    model: InferencePredictor | None = None,
    ...
    n_y : int, default=1
        Past Observable Frame steps in the measurement history window.
    n_u : int, default=0
        Past Control Current steps in the control history window.
    model : InferencePredictor | None, optional
        Loaded Predictor used to infer ``n_y`` and ``n_u`` if provided.
    ...
    valid_decisions : IntArray
        Indices of decision steps executed outside the Warm-up Period and with full Priming.
    """
    if model is not None:
        n_y = getattr(model, "n_y", n_y)
        n_u = getattr(model, "n_u", n_u)

    with np.load(log_file) as data:
        all_pred_y = np.asarray(data["controller.predicted_y"], dtype=np.float64)
        all_plan_u = np.asarray(data["controller.planned_u"], dtype=np.float64)
        all_applied_u = (
            np.asarray(data["controller.u"], dtype=np.float64) if "controller.u" in data else all_plan_u[:, 0, :]
        )
        warmup = (
            np.asarray(data["controller.warmup"], dtype=bool)
            if "controller.warmup" in data
            else np.zeros(len(all_pred_y), dtype=bool)
        )

    required_priming = max(n_y - 1, n_u - 1, 0)
    valid_idx = np.asarray(
        [d for d in range(len(all_pred_y)) if not warmup[d] and d >= required_priming],
        dtype=np.intp,
    )
    if len(valid_idx) == 0:
        return np.empty((0, 0, 0), dtype=np.float64), np.empty((0, 0, 0), dtype=np.float64), np.empty(0, dtype=np.intp)

    valid_plan_u = all_plan_u[valid_idx]
    horizon = valid_plan_u.shape[1]

    queries_list = [
        np.stack(
            [
                _build_mpc_knot_query(all_pred_y, all_plan_u, all_applied_u, d, k, n_y=n_y, n_u=n_u)
...
def _build_rollout_knot_query(  # noqa: PLR0913 -- knot extraction requires trajectory, rollout sequence, knot index, and window sizes
    y_arr: FloatArray,
    u_arr: FloatArray,
    u_seq: FloatArray,
    pred_seq: FloatArray,
    *,
    ...
    """Construct one state-action query vector along a multi-step rollout."""
    if n_y_feat <= 1:
        y_wk = y_arr[anc] if k == 0 else pred_seq[k - 1]
    else:
        y_parts = [
            y_arr[anc + step_offset] if step_offset <= 0 else pred_seq[step_offset - 1]
            for step_offset in range(k - (n_y_feat - 1), k + 1)
        ]
        y_wk = np.concatenate(y_parts, axis=0)

    if n_u_feat <= 0 or n_u_feat == 1:
        u_wk = u_seq[k]
    else:
        u_parts = [
            u_arr[anc + step_offset] if step_offset < 0 else u_seq[step_offset]
            for step_offset in range(k - (n_u_feat - 1), k)
        ]
        u_parts.append(u_seq[k])
        u_wk = np.concatenate(u_parts, axis=0)

    return np.concatenate([y_wk.reshape(-1), u_wk.reshape(-1)])
...
def evaluate_prediction_errors(  # noqa: PLR0913, PLR0915 -- multi-step and one-step evaluation share anchor alignment
    ...
    *,
    ...
    n_y : int | None, optional
        Past Observable Frame steps used in state-action features. Inferred from ``model`` if None.
    n_u : int | None, optional
        Past Control Current steps used in state-action features. Inferred from ``model`` if None.
    ...
    """
    n_y_model = getattr(model, "n_y", 1)
    n_u_model = getattr(model, "n_u", 0)
    n_y_feat = n_y_model if n_y is None else n_y
    n_u_feat = n_u_model if n_u is None else n_u
    ...
    for u_raw, y_raw in trajectories:
        ...
        t_len = min(len(u_arr), len(y_arr))
        start_anchor = max(n_y_model - 1, n_u_model, n_y_feat - 1, n_u_feat - 1, 0)
        end_anchor = t_len - 1
        ...
        y_hist = np.stack([y_arr[t - n_y_model + 1 : t + 1] for t in anchors_1step], axis=0)
        u_hist = (
        ...
        z_1step_list: list[FloatArray] = []
        for t in anchors_1step:
            y_w = y_arr[t] if n_y_feat <= 1 else y_arr[t - n_y_feat + 1 : t + 1].reshape(-1)
            u_w = u_arr[t] if n_u_feat <= 0 or n_u_feat == 1 else u_arr[t - n_u_feat + 1 : t + 1].reshape(-1)
            z_1step_list.append(np.concatenate([y_w.reshape(-1), u_w.reshape(-1)]))

        z_1step = np.stack(z_1step_list, axis=0)
        ...
        if max_multi_anchor > start_anchor:
            ...
            y_hist_m = np.stack([y_arr[t - n_y_model + 1 : t + 1] for t in anchors_multi], axis=0)
            u_hist_m = (
            ...
            for i, anc in enumerate(anchors_multi):
                pred_seq = preds_multi[i]
                u_seq = u_arr[anc : anc + horizon]
                rollout_z_items = [
                    _build_rollout_knot_query(
                        y_arr, u_arr, u_seq, pred_seq, anc=anc, k=k, n_y_feat=n_y_feat, n_u_feat=n_u_feat
                    )
                    for k in range(horizon)
                ]

                z_rollout = np.stack(rollout_z_items, axis=0)
...
def select_ood_candidate_states(  # noqa: PLR0913 -- candidate selection requires run, thresholds, index, and intervals
    ...
    *,
    ...
    n_y : int, default=1
        Past Observable Frame steps used when extracting queries from ``log.npz``.
    n_u : int, default=0
    ...
    if ood_scores is None:
        ...
        queries, _, valid_decisions = extract_mpc_rollout_queries(log_file, n_y=n_y, n_u=n_u, model=model)
        q0 = queries[:, 0, :]
