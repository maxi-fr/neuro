...
def load_roast_3d_field_projection(
    path: str | Path,
) -> tuple[FloatArray, StrArray, StrArray, FloatArray]:
    """Load a 3D ROAST electric field projection NPZ file: electrode current (mA) -> E-field (V/m) per region.
    ...
    projection_E
        ...
        an arbitrary zero-sum montage is ``u @ projection_E``.
    channel_labels
        Control channel labels of shape ``(n_controls,)`` (e.g. 62 scalp + Ex8).
    region_labels
        Region labels of shape ``(n_nodes,)``, in the plant's node order.
    region_normals
        Outward unit region surface normals of shape ``(n_nodes, 3)``, in the field projection's MNI
        RAS frame.
    """
    p = Path(path)
    if not p.exists():
        msg = f"3D Field projection file not found at {p}"
    ...
    with np.load(p) as data:
        if "projection_E" in data:
            ...
            msg = f"NPZ file at {p} carries neither 'projection_E', 'leadfield_E' nor legacy 'leadfield_3d'"
            raise KeyError(msg)
        ...
        if "region_normals" not in data:
            msg = (
                f"{p} carries no region_normals. The field projection and the normals it is reduced "
    ...
    return projection_E, channel_labels, region_labels, region_normals


class Roast3DStim(StimulationModel):
    """tES field from a ROAST 3D FEM field projection, reduced along each region's cortical normal.

    The field projection's E-vectors and the region normals are both in MNI RAS; the file must carry
    ...
    def __init__(self, cfg: _Roast3DConfig, region_labels: StrArray) -> None:
        """Load the field projection, check it against ``region_labels`` and reduce the montage rows."""
        projection_E, channel_labels, file_regions, normals = load_roast_3d_field_projection(cfg.field_projection_path)
        ...
        if normals.shape != (n_nodes, _NDIM_3D) or not np.any(normals):
            msg = (
                f"{cfg.field_projection_path} carries no usable region_normals (shape {normals.shape}); "
                "the field projection must be regenerated with its own MNI RAS normals."
            )
            raise ValueError(msg)

        rows = select_rows(channel_labels, cfg.electrodes) if cfg.electrodes is not None else slice(None)
        selected_E = projection_E[rows]
        self.gamma = self.polarization_length_mm * np.einsum("kid,id->ki", selected_E, normals)
