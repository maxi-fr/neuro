...
class ObservableMLPModel(DiscreteDynamics, InferencePredictor):
    """trajopt ``DiscreteDynamics`` adapter for the observable MLP predictor, on the Frame grid.

    Holds the checkpoint's float64 weights, per-output standardizer buffers and Observable
    geometry as jax arrays, so the model rolls one MLP ``step`` per call with no torch in the
    loop. Advances one Frame per step under the Control Current held over that hop.
    """
    ...
    def __init__(  # noqa: PLR0913 -- architecture record, standardizers, geometry and weights are the model
        self,
        *,
        n_y: int,
        n_u: int,
    ...
    def _predict(self, y_window: jax.Array, u_window: jax.Array) -> jax.Array:
        ...
        With the residual skip the MLP output adds the window's last Frame, so the layers fit
        the one-step delta exactly as the torch module does.
        """
        z = _mlp(
            self.activation,
    ...
    def discrete_dynamics(
        ...
        """Advance one Frame: shift held control ``u`` into the control window, predict, and shift both windows."""
        del t, dt
        n_z = self.n_y * self.n_outputs
        y_window = x[:n_z].reshape(self.n_y, self.n_outputs)
        u_window_raw = x[n_z:].reshape(self.n_u, self.n_controls)
    ...
    def _rollout_one(self, y_hist: jax.Array, u_hist: jax.Array, u_future: jax.Array) -> jax.Array:
        """Free-run one raw history under raw future controls -> raw ``(steps, n_outputs)``."""
        y_hist_flat = y_hist.reshape(y_hist.shape[0], self.n_outputs)
        y_window = (y_hist_flat[-self.n_y :] - self.y_center) / self.y_scale
        u_window = (u_hist[-self.n_u :] - self.u_center) / self.u_scale
        u_future = (u_future - self.u_center) / self.u_scale
    ...
    def absorb(self, state: FloatArray, y: FloatArray, u: FloatArray) -> FloatArray:
        """Append raw Frame ``y`` and applied control ``u`` into the shift-register state."""
        n_z = self.n_y * self.n_outputs
        state_arr = np.asarray(state, dtype=np.float64)
        y_window = state_arr[:n_z].reshape(self.n_y, self.n_outputs)
        u_window = state_arr[n_z:].reshape(self.n_u, self.n_controls)
    ...
    def is_ready(self, state: FloatArray) -> bool:
        """Report whether the output window holds no NaN, i.e. at least ``n_y`` Frames were absorbed."""
        n_z = self.n_y * self.n_outputs
        return not bool(np.isnan(np.asarray(state, dtype=np.float64)[:n_z]).any())
