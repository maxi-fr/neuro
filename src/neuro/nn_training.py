...
def _artifact_from_eqx(
    ...
    """Pull ``(weight, bias)`` out of a trained Equinox predictor into a framework-free artifact."""
    layers = []
    ...
    return MLPArtifact(
        layers=tuple(layers),
        activation=cast("Activation", model.activation),
        n_y=model.n_y,
        n_u=model.n_u,
...
def train_and_save_predictor(  # noqa: PLR0915
    ...
    *,
    ...
    then writes ``model.npz``, ``loss_curve.png``, ``training_stats.json`` and ``comparison.png``
    into ``artifact_dir``.
    ...
    data = prepare_datasets(
        data_files, n_steps_cfg, downsample, n_y, n_u, horizon, sim_cfg.dt, train_split, cutoff_hz=sim_cfg.cutoff_hz
    )
    ...
    if not isinstance(model, AutoregressivePredictor):
        ...
        raise TypeError(msg)
    art = _artifact_from_eqx(model, float(dt_real), int(downsample), y_pipeline, u_pipeline)
    art.save(artifact_dir / "model")
    ...
    Y_pred_unflat = Y_pred_raw_flat.reshape(-1, horizon, n_channels)
