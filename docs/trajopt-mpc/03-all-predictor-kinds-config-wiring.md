...
**What to build:** Extend ticket 01's `AbstractModel` pattern to the Observable predictor
kind, and wire the resulting controller into the existing config/class-path dispatch pattern.
A `Problem`-building factory function per predictor kind (waveform/Observable) assembles the
...
either of the two predictor kinds, the same way it dispatches `neuro.control.nonlinear_mpc.
MPCController` today.
...
- [ ] Observable predictor `AbstractModel` adapter exists, rollout-parity tested
      against the corresponding torch Predictor's `rollout`, same tolerance bar as ticket 01's
      waveform adapter.
- [ ] The new adapter is drivable through the ticket 01/02 controller with no controller-side
      changes needed.
...
- [ ] `configs/simulation/*mpc*.yaml` can dispatch `class_path` at the new controller and the
      appropriate factory function for either predictor kind, with no new config schema.
